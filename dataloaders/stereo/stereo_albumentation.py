...
def horizontal_flip(img_left, img_right, disp_left, disp_right):
    """
    ...
    if p >= 0.5:
        left_flipped = img_left[:, ::-1].copy()
        right_flipped = img_right[:, ::-1].copy()
        img_left = right_flipped
        img_right = left_flipped

        disp = disp_right[:, ::-1].copy()
        disp_right = disp_left[:, ::-1].copy()
    else:
        disp = disp_left
